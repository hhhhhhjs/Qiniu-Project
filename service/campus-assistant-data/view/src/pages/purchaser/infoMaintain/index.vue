<template>
  <div class="page-content-s-i">
      ...
      <div id="anchor_content" class="anchor_content">
        <div>
          <div id="info_id1" class="anchor_item">
            <span>基本信息</span>
            <a-button type="primary" size="small" v-if="formDataBasic.supplierStatus !== '2' && formDataBasic.supplierStatus !== '3'" 
        ...
        <div>
          <div id="info_id2" class="anchor_item">
            <span>经营范围</span>
            <a-button type="primary" ghost size="small" v-if="formDataBasic.supplierStatus !== '2' && formDataBasic.supplierStatus !== '3'" 
    ...
    <!-- 经营范围 -->
    <addEditABusiness :visible="columns2DialogVisible" :type="columns2DialogType" :form-data="columns2Data"
      @handle-cancel="columns2DialogVisible = false;" @handle-ok="addEditColumns2Execute"></addEditABusiness>
    <!-- 资质文件 -->
    <addEditBQualify :visible="columns3DialogVisible" :type="columns3DialogType" :form-data="columns3Data"
      @handle-cancel="columns3DialogVisible = false;" @handle-ok="addEditColumns3Execute"></addEditBQualify>
    <!-- 专利技术 -->
    <addEditCPatent :visible="columns4DialogVisible" :type="columns4DialogType" :form-data="columns4Data"
      @handle-cancel="columns4DialogVisible = false;" @handle-ok="addEditColumns4Execute"></addEditCPatent>
    <!-- 社会诚信 -->
    <addEditDSocial :visible="columns5DialogVisible" :type="columns5DialogType" :form-data="columns5Data"
      @handle-cancel="columns5DialogVisible = false;" @handle-ok="addEditColumns5Execute" ref="addEditDSocialRef">
    </addEditDSocial>
    <!-- 供应清单 -->
    <addEditEList :visible="columns6DialogVisible" :type="columns6DialogType" :form-data="columns6Data"
      @handle-cancel="columns6DialogVisible = false;" @handle-ok="addEditColumns6Execute"></addEditEList>
    <!-- 删除弹框 -->
...
// <!-- "3":已认证 -->
// <!-- "4":未通过 -->
const submitAudit = (supplierStatus:string) => {
  if (supplierStatus == '2') {
    companyApi.putCompanyAuditStatus(supplierStatus).then((res:any) => {
      if (res.success) {
        ...
        companyApi.putCompanyAuditStatus('3').then();
