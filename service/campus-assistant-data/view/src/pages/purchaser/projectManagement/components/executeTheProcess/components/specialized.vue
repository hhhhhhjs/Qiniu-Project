...
<template>
                ...
                <div class="img-list">
                    ...
                    <div class="img-obj" v-if="!disabledFlag">
                        <div class="picture-card" @click="openDialog">
                            ...
                            <div class="upload-text">邀请专家</div>
                        </div>
                    </div>
    ...
    <a-modal v-model:open="modalDialog" :title="'邀请专家'" @ok="formOk" :destroyOnClose="true">
        <a-form :model="formState" ref="formRef" :rules="formRules">
            <a-form-item label="专家" name="companyId">
        ...
        <div class="del-dialog-style">
            ...
            <div class="del-style-body">
                ...
                <div class="del-style-body-bottom">
                    <!-- 请确认删除该 -->
                    {{ $t('placeholder.pleaseConfirmTheDeletion') }}专家:{{ delItem.name }}
                </div>
            </div>
        </div>
    </a-modal>
...
//评审人员
import { rviewerApi } from '@/api/ExecuteRviewer'
import type { rviewer } from '@/api/ExecuteRviewer'
//专家库
import { reviewExpertsApi } from '@/api/ReviewExperts'
...
const formRules = {
    companyId: [{ required: true, message: '专家不可为空' }],
}
const openDialog = () => {
...
const formOk = () => {
    ...
    formRef.value.validate().then((res: any) => {
        const data = <rviewer>{
            reviewId: formState.value.companyId,
            executeId: props.propsid
        }
        rviewerApi.addrviewer(data).then((res: any) => {
            if (res.success) {
...
const delOk = () => {
    ...
    rviewerApi.delrviewer(item.id).then((res: any) => {
        if (res.success) {
...
const getCompanyNameList = () => {
    if (!props.propsid) {
        ...
        rviewerApi.getRviewerList(props.propsid).then((res:any)=>{
            if(res.success){
                companyNameList.value = res.obj
                setTimeout(() => {
                    companyNameList.value.forEach((item: any) => {
                        companyList.value.forEach((ele: any) => {
                            if (ele.value == item.reviewId) {
                                ele.disabled = true
                            }
                        })
                    })
...
//获取专家库
const getReviewExpertsList = () => {
    reviewExpertsApi.getReviewExpertsList().then((res: any) => {
        if (res.success) {
            companyList.value = []
